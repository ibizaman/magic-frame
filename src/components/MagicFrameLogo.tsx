...
export default function MagicFrameLogo({ className = "w-12 h-12" }: { className?: string }) {
    // Unique gradient id per instance — the logo renders multiple times
    // (desktop sidebar, mobile topbar, mobile menu). A fixed id="brandGrad"
    // duplicated across instances makes url(#brandGrad) resolve to the first
    // (often display:none) one, so the gradient frame fails to paint and the
    // logo looks black. useId keeps each instance self-contained + SSR-safe.
    ...
    return (
        <svg className={className} viewBox="0 0 120 160" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect x="5" y="5" width="110" height="150" rx="22" fill={`url(#${gradId})`} />
            <rect x="14" y="14" width="92" height="132" rx="14" fill="#0f172a" />

            <path d="M14 115 L38 80 L60 100 L88 70 L106 90 V146 H14 Z" fill={`url(#${gradId})`} fillOpacity="0.4" />
