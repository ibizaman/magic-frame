...
export default function WeatherInspector({
  ...
  return (
    <div className="space-y-4">
       <div>
          <label className="text-sm font-medium text-white/80 block mb-2 text-emerald-400">{t("Datenquelle")}</label>
          <select
             value={provider}
             onChange={(e) => updateConfig(activeWidget.i, 'provider', e.target.value)}
             className="w-full bg-black border border-emerald-500/30 text-white text-sm rounded-lg p-3 focus:outline-none focus:border-emerald-400"
          >
       ...
       {provider === "home-assistant" && (
          <div>
             <label className="text-sm font-medium text-white/80 block mb-2">{t("HA-Entity-ID")}</label>
             <input
                type="text"
                value={(activeWidget.config as any)?.haEntity || ""}
                placeholder="weather.home"
                onChange={(e) => updateConfig(activeWidget.i, 'haEntity', e.target.value)}
                className="w-full bg-black border border-white/10 text-white font-mono text-sm rounded-lg p-3 focus:outline-none focus:border-emerald-500"
             />
             <p className="text-[11px] text-white/40 mt-1">
       ...
       <div className={`relative ${provider === "home-assistant" ? "opacity-50 pointer-events-none" : ""}`}>
          <label className="text-sm font-medium text-white/80 block mb-2 text-cyan-400">{t("Ort suchen (Auto-Ausfüllen)")}</label>
          {(activeWidget.config as any)?.location && (
             <div className="flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/30 rounded-lg px-3 py-2 mb-2">
                ...
                <div className="flex-1 min-w-0">
                   <div className="text-xs text-white/60 uppercase tracking-wider">{t("Aktueller Ort")}</div>
                ...
                <button
                   ...
                   className="text-xs text-white/50 hover:text-red-300 hover:bg-red-500/10 rounded w-7 h-7 flex items-center justify-center shrink-0"
                >
                   ×
                </button>
             </div>
       ...
       <div className="pt-4 mt-4 border-t border-white/10 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
             <label className="text-xs font-medium text-white/40 block mb-2">{t("Latitude")}</label>
             <input
                type="text" value={activeWidget.config?.lat || ''} placeholder="52.5200"
                onChange={(e) => updateConfig(activeWidget.i, 'lat', e.target.value)}
                className="w-full bg-black border border-white/5 text-white/50 font-sans text-sm rounded-lg p-2 focus:outline-none focus:border-white/20"
             />
          </div>
          <div>
             <label className="text-xs font-medium text-white/40 block mb-2">{t("Longitude")}</label>
             <input
       ...
       <div className="pt-4 mt-4 border-t border-white/10 space-y-4">
          <div>
             <label className="text-sm font-medium text-white/80 block mb-2 text-orange-400">{t("Ort-Label (über der Temperatur)")}</label>
             <div className="grid grid-cols-2 gap-3">
                <div>
                   ...
                   <input
                      ...
                      className="w-full h-2 rounded-lg appearance-none cursor-pointer accent-orange-500 bg-white/10"
                   />
                </div>
                <div>
                   <label className="text-xs font-medium text-white/60 flex justify-between mb-1.5">
             ...
             <div className="grid grid-cols-2 gap-3 mt-3">
                <label className="flex items-center gap-2 cursor-pointer text-xs text-white/70 hover:text-white">
                   <input
                      ...
                      className="appearance-none w-4 h-4 border border-white/20 rounded bg-black checked:bg-orange-500 checked:border-orange-500"
                   />
                   {t("GROSSBUCHSTABEN")}
                </label>
                <select
                   ...
                   className="bg-black border border-white/10 text-white text-xs rounded-md px-2 h-8 focus:outline-none focus:border-orange-500"
                >
          ...
          <div className="grid grid-cols-2 gap-3">
             <div>
                <label className="text-xs font-medium text-white/70 block mb-1.5">{t("Temperatur")}</label>
                <select
             ...
             <div>
                <label className="text-xs font-medium text-white/70 block mb-1.5">{t("Wind")}</label>
                <select
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             <div>
                <label className="text-sm font-medium text-white/80 block mb-2">{t("Ansicht (Layout)")}</label>
                <select
                   value={activeWidget.config?.forecastLayout || 'horizontal'}
                   onChange={(e) => updateConfig(activeWidget.i, 'forecastLayout', e.target.value)}
                   className="w-full bg-black border border-white/10 text-white font-sans text-sm rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none cursor-pointer"
                >
             ...
             <div>
                <label className="text-sm font-medium text-white/80 block mb-2">{t("Icon Stil")}</label>
                <select
          ...
          <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
             <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm font-medium text-white/80 group-hover:text-white transition-colors">{t("Tages-Vorhersage ausblenden")}</span>
             </label>
          ...
          <div className="mt-3 pt-3 border-t border-white/10">
             <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm font-medium text-white/80 group-hover:text-white transition-colors">{t("Stündlichen Verlauf anzeigen")}</span>
             </label>
             ...
             <div className="grid grid-cols-2 gap-3 mt-3">
                <label className="flex items-center gap-2 cursor-pointer text-xs text-white/70 hover:text-white">
                   <input
                      ...
                      className="appearance-none w-4 h-4 border border-white/20 rounded bg-black checked:bg-blue-500 checked:border-blue-500"
                   />
                   {t("GROSSBUCHSTABEN")}
                </label>
                <select
                   ...
                   className="bg-black border border-white/10 text-white text-xs rounded-md px-2 h-8 focus:outline-none focus:border-blue-500"
                >
                   <option value="normal">{t("Normaler Abstand")}</option>
                   <option value="wide">{t("Weiter Abstand")}</option>
                   <option value="widest">{t("Sehr weiter Abstand")}</option>
       ...
       <div>
          <label className="text-xs font-medium text-white/40 block mb-2">{t("Anzeigename auf Dashboard")}</label>
          <input
             ...
             className="w-full bg-black border border-white/5 text-white font-sans text-sm rounded-lg p-3 focus:outline-none focus:border-white/20"
          />
        </div>
    </div>
  );
